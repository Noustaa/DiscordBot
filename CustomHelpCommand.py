...
class CustomHelpCommand(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        for cog in mapping:
            await self.get_destination().send(f'{cog.qualified_name}: {[command.name for command in mapping[cog]]} - {[command.description for command in mapping[cog]]}')

    # method that is called when executing !help <cog_name>, it will list all available commands in a specific cog
    # and their descriptions
    async def send_cog_help(self, cog):
        await self.get_destination().send(f'{cog.qualified_name}: {[command.name for command in cog.get_commands()]} - {[command.description for command in cog.get_commands()]}')
